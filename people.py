...
class Person:
    def __init__(self, id, pos_x, pos_y, category):
        self.id = id
        self.pos = (pos_x, pos_y)
        self.category = category
        self.speed = self.set_speed(category)
        self.savety = False

    def set_speed(self, category):
        speeds = {'adult_male': 1.19, 'adult_female': 0.98,
                  'elderly': 0.55, 'child': 0.64}
        return speeds.get(category, 1.0)  # 默认速度为1.0 m/s
...
class People:
    def __init__(self, cnt, myMap):
        ...
        # 根据论文中的比例生成不同种类的人群
        categories = {
            'adult_male': int(cnt * 0.27),
            'adult_female': int(cnt * 0.25),
            'elderly': int(cnt * 0.40),
            'child': int(cnt * 0.08)
        }
        ...
        for category, num in categories.items():
            for _ in range(num):
                ...
                if pos_x >= 26:
                    self.list.append(Person(current_cnt + 1, pos_x, pos_y, 'adult_male'))
                    current_cnt += 1

        # 如果由于四舍五入造成的总数不足，添加额外的成年男性
