...
class Map:
    ...
    def display_map(self):
        ...
        ax.set_xlim(-30, self.Length + 2 * Outer_Size)
        ax.set_ylim(-30, self.Width + 2 * Outer_Size)

        # Draw the exits
        for ex, ey in self.Exit:
            ax.add_patch(patches.Rectangle((ex - 0.5, ey - 0.5), 1, 1, color='green'))

        # Draw the barriers
        for barrier in self.Barrier:
            bottom_left = barrier[0]
            top_right = barrier[1]
            width = top_right[0] - bottom_left[0] + 1
            height = top_right[1] - bottom_left[1] + 1
            ax.add_patch(patches.Rectangle(bottom_left, width, height, color='red'))
...
# 出口
# 点集
Exit = Init_Exit(P1=(0, 28), P2=(0, 65))


# 障碍 矩形区域
Barrier = list()
Barrier.append(Init_Barrier(A=(40, 35), B=(77, 65)))
Barrier.append(Init_Barrier(A=(6, 49), B=(22, 94)))
Barrier.append(Init_Barrier(A=(22, 79), B=(62, 94)))
Barrier.append(Init_Barrier(A=(62, 79), B=(118, 94)))
Barrier.append(Init_Barrier(A=(103, 0), B=(118, 83)))
Barrier.append(Init_Barrier(A=(82, 0), B=(103, 30)))
Barrier.append(Init_Barrier(A=(44, 0), B=(82, 10)))
Barrier.append(Init_Barrier(A=(26, 0), B=(44, 26)))
